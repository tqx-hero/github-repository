...
int main(){
	...
	//拼装请求数据的报文：
	char buffer[2048]={0};
	int len = sprintf(buffer,"%c%c%s%c%s%c%s%c%s%c",0x00,TFTP_RD,"test.txt",0x00,transformat,0x00,"blksize",0x00,"1024",0x00);
	...
	while(1){
		...
		if(op == TFTP_DATA){
			...
			*(uint16_t*)(buffer+2) = block_num;
			...
			printf("%s\n",buffer+4);
			break;
		}else if(op == TFTP_OACK){
			for(int i=0;i<recv_len;i++){
				printf("%x ",*(buffer+i));	
			}
			printf("\n");
			*(uint16_t*)buffer = htons(TFTP_ACK);
			*(uint16_t*)(buffer+2) = htons(TFTP_ACKO);
			sendto(sock_fd,buffer,4,0,(struct sockaddr *)&addr,sizeof(addr));
		}
	}
	fclose(fl_ptr);
