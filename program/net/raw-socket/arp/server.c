 ...
 *地址解析协议ARP
 */
int init_frame_header(uint8_t*,uint8_t *);
int init_arp_data(uint8_t*,uint8_t* ,uint8_t *,uint8_t* ,int);
int main(int argc,char ** argv,char** env){
	...
	frame_size+= init_arp_data(frame_buffer+frame_size,mac_addr,src_ip,dst_ip,sizeof(src_ip));	
	//发送arp报文
