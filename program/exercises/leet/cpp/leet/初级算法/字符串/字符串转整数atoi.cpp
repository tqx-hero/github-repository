...
class Solution {
    bool out_of_bound(long long number) {
...
public:
    int myAtoi(string s) {
        long long ret = 0;
        int symbol = 1;
        string::iterator begin_iter = s.begin(), end_iter = s.end();
        //过滤掉无用的前导空格，找到第一个有用字符
        while (begin_iter != end_iter && *begin_iter == ' ')
            ++begin_iter;
        char ch = *begin_iter;
        //第一位数字是否是+、-或者数字1~9
        bool is_digit = false;
        if (ch == '+' || ch == '-' || (is_digit = isdigit(ch))) {
            if (ch == '-')
                symbol = -1;
            else if (is_digit)
                ret = ch - '0';
        ...
        for (++begin_iter; begin_iter != end_iter; ++begin_iter) {
            if (isdigit(*begin_iter)) {
                ret = ret * 10 + (*begin_iter - '0');
                if (out_of_bound(ret * symbol))
