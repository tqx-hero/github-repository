...
public:
    vector<int> findMissingElements(vector<int> &nums) {
        ...
        size_t size = nums.size(), capacity;
        if ((capacity = nums[size - 1] + 1 - nums[0]) == size)
            return result;
        size_t resSize = capacity - size;
        result.reserve(resSize);
        int begin = nums[0];
        for (int i = 0; result.size() < resSize; i++, begin++) {
            //如果不相等，该数据为缺失数据,添加到result
            int cur = nums[i];
            while (begin != cur) {
...
//     Solution sl;
//     const auto &result = sl.findMissingElements(nums);
